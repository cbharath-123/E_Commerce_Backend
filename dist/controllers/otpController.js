...
exports.resendSellerOTP = exports.verifySellerOTP = exports.generateSellerOTP = void 0;
const app_1 = require("../app");
const otpGenerator_1 = require("../utils/otpGenerator");
// Generate OTP for seller verification
const generateSellerOTP = async (req, res) => {
    try {
        ...
        const recentOTPs = await app_1.prisma.otpVerification.count({
            where: {
                userId,
                createdAt: {
                    gte: oneHourAgo
        ...
        const expiresAt = (0, otpGenerator_1.generateOTPExpiry)();
        const hashedOTP = (0, otpGenerator_1.hashOTP)(otpCode);
        // Clear any existing OTPs for this user
        await app_1.prisma.otpVerification.deleteMany({
            where: { userId }
        });
        // Store new OTP
        await app_1.prisma.otpVerification.create({
            data: {
                userId,
                otpCode: hashedOTP,
                expiresAt
...
exports.generateSellerOTP = generateSellerOTP;
// Verify OTP code
const verifySellerOTP = async (req, res) => {
    try {
        ...
        const otpRecord = await app_1.prisma.otpVerification.findFirst({
            where: {
                userId,
                verified: false
            },
        ...
        if (new Date() > otpRecord.expiresAt) {
            await app_1.prisma.otpVerification.delete({
        ...
        // Verify OTP
        const isValid = (0, otpGenerator_1.verifyOTP)(otpCode, otpRecord.otpCode);
        if (!isValid) {
            // Increment attempts
            await app_1.prisma.otpVerification.update({
                where: { id: otpRecord.id },
        ...
        const sellerToken = jwt.sign({
            ...
            otpVerified: true,
            sessionType: 'seller'
        }, process.env.JWT_SECRET, { expiresIn: '24h' } // Shorter expiry for seller sessions
        );
        res.json({
...
const resendSellerOTP = async (req, res) => {
    try {
        ...
        const recentOTP = await app_1.prisma.otpVerification.findFirst({
            where: {
                userId,
                createdAt: {
                    gte: twoMinutesAgo
        ...
        return (0, exports.generateSellerOTP)(req, res);
    }
    catch (error) {
...
exports.resendSellerOTP = resendSellerOTP;
